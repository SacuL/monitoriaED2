...
void imprimeEstrutura::percorreBinaria(No* p, float x, float y, float desX){
    ...
    if(p->getEsquerda() != NULL){
        ...
        pos1[0] = pos2[0] - desX;
        pos1[1] = pos2[1] - desY;
        percorreBinaria(p->getEsquerda(), pos1[0], pos1[1], desX);

    }if(p->getDireita() != NULL){

        pos1[0] = pos2[0] + desX;
        pos1[1] = pos2[1] - desY;
        percorreBinaria(p->getDireita(), pos1[0], pos1[1], desX);
...
int imprimeEstrutura::alturaArvoreBinaria(No* p, int nivel){
    ...
    if(p == NULL){
        ...
        esq = alturaArvoreBinaria(p->getEsquerda(),nivel+1);
        dir = alturaArvoreBinaria(p->getDireita(),nivel+1);
        if(esq > dir){
