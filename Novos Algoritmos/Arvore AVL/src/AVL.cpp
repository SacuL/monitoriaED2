...
No* AVL::InsereAVL(No* p, int ch){
	...
	else
	{
		if(ch < p->getValor())
		{
			p->setAnt(InsereAVL(p->getAnt(),ch));
		}
		else
		{
			p->setProx(InsereAVL(p->getProx(),ch));
		}
	}
	return p;
}
...
void AVL::insere(int p){

    raiz = InsereAVL(raiz, p);
