    ...
    if(tipoEstrutura == 1){
        ...
        if(PATTREE == NULL){
            ...
            if(*PATTREE != NULL){
                ...
                percorrePatricia(*PATTREE, pos[0], pos[1],altura);
...
int imprimeEstrutura::alturaArvore(){
    ...
    }else if(tipoEstrutura == ARVORE_PATRICIA){

        if(*PATTREE != NULL){

            return alturaArvorePatricia(*PATTREE);
...
imprimeEstrutura::imprimeEstrutura(const int TIPO){
    ...
    cout<<"Versao: 1.3 -> 12-07-2016"<<endl<<endl;
    cout<<"Comandos: "<<endl;
...
void imprimeEstrutura::setPriArvorePatricia(No_PAT** p){

    PATTREE = p;

}
void imprimeEstrutura::percorrePatricia(No_PAT* p, float x, float y, float desX){
    ...
    if(p != NULL){
        ...
        posP[1] = y;
        posAux[1] = y-5;
        posAuxPai[0] = x-p->getTamAlfa()/2;
        posAuxPai[1] = y-1;
        float distX = pow(p->getTamAlfa(),desX);
        desenhaNoPAT(posP,p->getChave(),p->getTamAlfa(), p->getPosPref());
        x = x - distX/2;
        distX /= (float)p->getTamAlfa();
        for(int i = 0; i <= p->getTamAlfa(); i++){

            if(i < p->getTamAlfa()){

                if(p->getFilho(97+i) != NULL){

                    posAux[0] = x;
                    desenhaLinha(posAuxPai,posAux);

                }
                percorrePatricia(p->getFilho(97+i),x,y-5.0,desX-1);
                posAuxPai[0] += 1.0;
                x += distX;

            }else{

                if(p->getFilho('\0') != NULL){
...
int imprimeEstrutura::alturaArvorePatricia(No_PAT *p){
    ...
    int maxAlt = 0;
    int aux = 0;
    for(char c = 'a'; c < 97 + p->getTamAlfa(); c++){

        aux = alturaArvorePatricia(p->getFilho(c));
        if(aux > maxAlt){
...
void imprimeEstrutura::desenhaNoPAT(float *posPAT, string valor, int tamAlfa, int posOrd){

    float desX = (tamAlfa+1)/2.0;
    if(posOrd != -1){
        ...
        glColor3f(0.0,0.0,1.0);
        glBegin(GL_QUADS);
            glVertex2f(posPAT[0]-desX,posPAT[1]-1.5);
        ...
        glColor3f(1.0,1.0,1.0);
        glBegin(GL_QUADS);
            glVertex2f(posPAT[0]-desX,posPAT[1]-0.1);
            glVertex2f(posPAT[0]-desX,posPAT[1]);
            glVertex2f(posPAT[0]+desX,posPAT[1]);
            glVertex2f(posPAT[0]+desX,posPAT[1]-0.1);
        glEnd();

        glBegin(GL_QUADS);
            glVertex2f(posPAT[0]-desX,posPAT[1]-0.85);
            glVertex2f(posPAT[0]-desX,posPAT[1]-0.75);
            glVertex2f(posPAT[0]+desX,posPAT[1]-0.75);
            glVertex2f(posPAT[0]+desX,posPAT[1]-0.85);
        glEnd();

        posP[0] = posPAT[0] - (tamAlfa+1)/2.0;
        posP[1] = posPAT[1];
        c[0] = 'a';
        c[1] = '\0';
        for(int i = 1; i <= tamAlfa; i++){
            ...
            glColor3f(1.0,1.0,1.0);
            glBegin(GL_QUADS);
                glVertex2f(posP[0]+i-0.05, posP[1]-1.5);
                glVertex2f(posP[0]+i-0.05, posP[1]);
                glVertex2f(posP[0]+i+0.05, posP[1]);
                glVertex2f(posP[0]+i+0.05, posP[1]-1.5);
            glEnd();
        ...
        ss << posOrd;
        escreve((char*)ss.str().c_str(),posPAT[0], posPAT[1]+0.12);
        ...
        glEnd();
        glColor3f(0.0,0.0,1.0);
        escreve((char*)valor.c_str(),posPAT[0]-desX+0.3,posPAT[1]);
