...
void imprimeEstrutura::desenhaNoVP(float* pos, string valor, No* no){

    int numSeg = 32;
    float segmento = 2*M_PI/(float)numSeg;
    float tamanhoValor;
    if(no->getCor() == VERMELHO){
...
void imprimeEstrutura::display(){
   ...
   glLoadIdentity();
   if(tipoEstrutura == 1 || tipoEstrutura == 2 || tipoEstrutura == 5 || tipoEstrutura == 4 || tipoEstrutura == 6 || tipoEstrutura == 7 || tipoEstrutura == 8){
...
void imprimeEstrutura::percorreBinaria(No* p, float x, float y, float desX){
    ...
    oss << p->getValor();
    valor = oss.str();
    if(tipoEstrutura == ARVORE_VERMELHO_PRETA){
         desenhaNoVP(pos2,valor,p);
    }else{
...
int imprimeEstrutura::alturaArvore(){
    ...
    if(tipoEstrutura == 1 || tipoEstrutura == 8){
...
void imprimeEstrutura::setHeapBinomial(No_Binomial** b){
