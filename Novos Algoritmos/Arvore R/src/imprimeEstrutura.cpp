    ...
    if(tipoEstrutura == 1){
        ...
        if(RTREE == NULL){
            ...
            if(*RTREE != NULL){
                ...
                pos1[0] = 10.0 + dx;
                pos1[1] = XYcart * ( 3.0 / 5.5 )  + dy;
                desenhaLinhasR(pos1,*RTREE,altura);
                percorreR(pos1,*RTREE,altura);
...
int imprimeEstrutura::alturaArvore(){
    ...
    }else if(tipoEstrutura == ARVORE_R){


        if(*RTREE != NULL){

            return alturaArvoreR(*RTREE);
...
imprimeEstrutura::imprimeEstrutura(const int TIPO){
    ...
    cout<<"Versao: 1.3 -> 24-06-2016"<<endl<<endl;
    cout<<"Comandos: "<<endl;
...
void imprimeEstrutura::desenhaNoR(float *posR, NoR* n){

    glColor3f(0.0,0.0,1.0);
    glBegin(GL_QUADS);///Desenha o retangulo azul

        glVertex2f(posR[0]-(MAX_R-1)/2.0,posR[1]-1.0);
        glVertex2f(posR[0]-(MAX_R-1)/2.0,posR[1]+1.0);
        glVertex2f(posR[0]+(MAX_R-1)/2.0,posR[1]+1.0);
        glVertex2f(posR[0]+(MAX_R-1)/2.0,posR[1]-1.0);
    ...
    glEnd();
    glColor3f(1.0,1.0,1.0);
    for(int i = 1; i < MAX_R-1; i++){///Desenha as linhas brancas

        glBegin(GL_QUADS);

            glVertex2f(posR[0]-(MAX_R-1)/2.0+i-0.05, posR[1]-1.0);
            glVertex2f(posR[0]-(MAX_R-1)/2.0+i-0.05, posR[1]);
            glVertex2f(posR[0]-(MAX_R-1)/2.0+i+0.05, posR[1]);
            glVertex2f(posR[0]-(MAX_R-1)/2.0+i+0.05, posR[1]-1.0);
    ...
    glBegin(GL_QUADS);///Desenha a linha branca horizontal

        glVertex2f(posR[0]-(MAX_R-1)/2.0,posR[1]-0.05);
        glVertex2f(posR[0]-(MAX_R-1)/2.0,posR[1]+0.05);
        glVertex2f(posR[0]+(MAX_R-1)/2.0,posR[1]+0.05);
        glVertex2f(posR[0]+(MAX_R-1)/2.0,posR[1]-0.05);

    glEnd();
    if(n->MBR != NULL){///Escreve a MBR do No 'n'

        escreve((char*)converteMBR(n->MBR).c_str(),posR[0],posR[1]+0.7);
        escreve((char*)converteMBR(n->MBR->getProx()).c_str(),posR[0],posR[1]+0.4);

    }else{

       escreve((char*)"NULL",posR[0],posR[1]+0.7);

    }
    for(int i = 0; i < MAX_R-1; i++){///escreve a MBR dos filhos

        if(n->filhos[i] != NULL){

            if(n->filhos[i]->MBR != NULL){

                escreve((char*)converteMBR(n->filhos[i]->MBR).c_str(),posR[0]-(MAX_R-1)/2.0+i+0.1,posR[1]-0.4);
                escreve((char*)converteMBR(n->filhos[i]->MBR->getProx()).c_str(),posR[0]-(MAX_R-1)/2.0+i+0.1,posR[1]-0.7);

            }else{

                escreve((char*)"NULL",posR[0]-(MAX_R-1)/2.0+i+0.1,posR[1]-0.4);
...
void imprimeEstrutura::setPriArvoreR(NoR** r){

    RTREE = r;

}
void imprimeEstrutura::percorreR(float *posR,NoR *n,float dist){

    if(!n->ehFolha()){

        desenhaNoR(posR,n);
        float posAux[2];
        posAux[1] = posR[1] - MAX_R-1;
        float distX = pow(MAX_R-1,dist);
        posAux[0] = posR[0] - distX/2;
        distX /= (float)MAX;
        for(int i = 0; i < MAX_R; i++){

            if(n->filhos[i] != NULL){
                posAux[0] += distX;
                percorreR(posAux,n->filhos[i],dist-1);
...
string imprimeEstrutura::converteMBR(Point* n){
...
int imprimeEstrutura::alturaArvoreR(NoR* n){
    ...
    for(int i = 0; i < MAX_R-1; i++){
        ...
        auxAlt = alturaArvoreR(n->filhos[i]);
        if(auxAlt > alt){
...
void imprimeEstrutura::desenhaLinhasR(float *posLin,NoR *n,float dist){

    if(!n->filhos[0]->ehFolha()){

        float posAux[2];
        posAux[1] = posLin[1] - MAX_R-1;
        float distX = pow(MAX_R-1,dist);
        posAux[0] = posLin[0] - distX/2;
        distX /= (float)MAX;
        for(int i = 0; i < MAX_R; i++){
            ...
            if(n->filhos[i] != NULL){
                ...
                posAux[0] += distX;
                desenhaLinha(posLin,posAux);
                desenhaLinhasR(posAux,n->filhos[i],dist-1);
